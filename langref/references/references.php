 ...
 *
 * 不用 & 运算符导致对象生成了一个拷贝。如果在类中用 $this，它将作用于该类当前的实例。
 ...
 * （它被表示为 $var，但是 $var 只有变量内容而没有调用符号表中的名字到值的绑定）。
 * 可以使用引用返回来引用被函数选择的变量。
 */
...
function bar2() // Note the missing &
{
    $a = 5;
    return $a;
}

foo2(bar2()); // 自 PHP 5.0.5 起导致致命错误，自 PHP 5.1.1 起导致严格模式错误
 ...
 * 同样也指出 $myValue 是作为引用的绑定，而不是通常的赋值。
 *
 * 如果试图这样从函数返回引用：return ($this->value);，这将不会起作用，因为在试图返回一个表达式的结果而不是一个引用的变量。
 * 只能从函数返回引用变量——没别的方法。如果代码试图返回一个动态表达式或 new 运算符的结果，
 * 自 PHP 4.4.0 和 PHP 5.1.0 起会发出一条 E_NOTICE 错误。
 */
